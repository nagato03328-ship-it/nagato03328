  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      if (typeof idea.id === 'number' || (typeof idea.id === 'string' && !idea.id.includes('-'))) {
        ...
        const mockLineData = Array.from({ length: idea.votes }, (_, i) => ({
          voteNumber: i + 1,
          yesVotes: Math.floor((i + 1) * 0.8),
          maybeVotes: Math.floor((i + 1) * 0.1),
          noVotes: Math.floor((i + 1) * 0.1)
        }));
        if (mockLineData.length === 0) mockLineData.push({ voteNumber: 0, yesVotes: 0, noVotes: 0, maybeVotes: 0 });
      ...
      try {
        ...
        if (data) {
          ...
          let currentNo = 0;
          let currentMaybe = 0;
          const newLineData = sortedData.map((vote, index) => {
          ...
          if (newLineData.length === 0) {
            newLineData.push({ voteNumber: 0, yesVotes: 0, noVotes: 0, maybeVotes: 0 });
          }
        ...
        setLineData([
          { voteNumber: 0, yesVotes: 0, noVotes: 0, maybeVotes: 0 }
        ]);
      }
    };
        ...
        return (
          <LineChart data={lineData}>
            ...
            <Tooltip 
              ...
              itemStyle={{ color: '#fff' }}
            />
            <Line type="monotone" dataKey="yesVotes" name="Yes" stroke="#10b981" strokeWidth={3} dot={false} />
            <Line type="monotone" dataKey="maybeVotes" name="Maybe" stroke="#eab308" strokeWidth={3} dot={false} />
            <Line type="monotone" dataKey="noVotes" name="No" stroke="#ef4444" strokeWidth={3} dot={false} />
          </LineChart>
        );
      case 'bar':
        return (
